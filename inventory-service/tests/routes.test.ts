...
describe('inventory-service', () => {
  ...
  it('GET /api/products?activeOnly=true returns only active', async () => {
    const res = await api.get('/api/products?activeOnly=true');
    expect(res.status).toBe(200);
  ...
  it('GET /api/products/sku/:sku returns product', async () => {
    const res = await api.get('/api/products/sku/LAPTOP-001');
    expect(res.status).toBe(200);
    expect(res.body.sku).toBe('LAPTOP-001');
  });

  it('GET /api/products/sku/:sku returns 404 for unknown sku', async () => {
    const res = await api.get('/api/products/sku/NOPE-999');
  ...
  it('GET /api/products/category/:category returns products', async () => {
    const res = await api.get('/api/products/category/Electronics');
    expect(res.status).toBe(200);
  ...
  it('POST /api/products creates a product', async () => {
    const before = (await api.get('/api/products')).body.length;
    const res = await api.post('/api/products').send(validProduct);
    expect(res.status).toBe(201);
    expect(res.body.sku).toBe(validProduct.sku);
    const after = (await api.get('/api/products')).body.length;
    expect(after).toBe(before + 1);
  });

  it('POST /api/products returns 400 for invalid payload', async () => {
    const res = await api.post('/api/products').send({ name: '' });
    expect(res.status).toBe(400);
  });

  it('POST /api/products returns 409 for duplicate SKU', async () => {
    await api.post('/api/products').send(validProduct);
    const res = await api.post('/api/products').send(validProduct);
  ...
  it('POST /api/products/:id/reserve decrements stock atomically', async () => {
    const create = await api.post('/api/products').send({ ...validProduct, stockQuantity: 10 });
    const id = create.body.id;
    const res = await api.post(`/api/products/${id}/reserve?quantity=3`);
    expect(res.status).toBe(200);
    expect(res.body.stockQuantity).toBe(7);
  });

  it('POST /api/products/:id/reserve returns 400 for insufficient stock', async () => {
    const create = await api.post('/api/products').send({ ...validProduct, stockQuantity: 2 });
    const id = create.body.id;
    const res = await api.post(`/api/products/${id}/reserve?quantity=5`);
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/insufficient stock/i);
  });

  it('POST /api/products/:id/reserve returns 404 for nonexistent product', async () => {
    const res = await api.post('/api/products/999999/reserve?quantity=1');
    expect(res.status).toBe(404);
  });
  ...
  it('POST /api/products/:id/restore increments stock', async () => {
    const create = await api.post('/api/products').send({ ...validProduct, stockQuantity: 5 });
    const id = create.body.id;
    const res = await api.post(`/api/products/${id}/restore?quantity=3`);
    expect(res.status).toBe(200);
  ...
  it('DELETE /api/products/:id deletes product', async () => {
    const create = await api.post('/api/products').send(validProduct);
    const id = create.body.id;
